...
if include_tracks == True and include_cascades == False and exist_track == True:
    ...
    raise RuntimeError("Must keep track or cascade or both events")
...
if "energy" in label_keys:
    ...
    boolarray = np.logical_and(np.array(labels["energy"]) > minenergy, np.array(labels["energy"]) < maxenergy)
    for k in label_keys:
        if k != "energy":
...
if "isTrack" in label_keys and include_tracks == False:
    ...
    boolarray = np.array(labels["isTrack"]) == False
    for k in label_keys:
        if k != "isTrack":
            labels[k] = labels[k][boolarray]
    for k in feature_keys:
...
if "isCascade" in label_keys and include_cascades == False:
    ...
    boolarray = np.array(labels["isCascade"]) == False
    for k in label_keys:
        if k != "isCascade":
            labels[k] = labels[k][boolarray]
    for k in feature_keys:
...
if "isCC" in label_keys and include_CC == False:
    ...
    boolarray = np.array(labels["isCC"]) == False
    for k in label_keys:
        if k != "isCC":
            labels[k] = labels[k][boolarray]
    for k in feature_keys:
...
if "isNC" in label_keys and include_NC == False:
    ...
    boolarray = np.array(labels["isNC"]) == False
    for k in label_keys:
        if k != "isNC":
            labels[k] = labels[k][boolarray]
    for k in feature_keys:
...
if only_reco == True and "reco" in fin:
    ...
    boolarray = np.array(reco["energy"]) != 0
    for k in label_keys:
        labels[k] = labels[k][boolarray]
    for k in feature_keys:
        features[k] = features[k][boolarray]
...
if flatten_energy == True:
    ...
    ranges = np.linspace(energymin, energymax, num=no_bins+1)

    #find energy bin with lowest number of entries
    energy_bin_length = []
    for i in range(no_bins):
        boolarray = np.logical_and(np.array(labels["energy"]) >= ranges[i], np.array(labels["energy"]) < ranges[i+1])
    ...
    energy_bin_length = np.array(energy_bin_length)
    ...
    for i in range(no_bins):
        boolarray = np.logical_and(np.array(labels["energy"]) >= ranges[i], np.array(labels["energy"]) < ranges[i+1])
        energy_indices = np.argwhere(boolarray).flatten()
        energy_indices = energy_indices[np.array(range(len(energy_indices))) > min_entries]

        for k in label_keys:
            labels[k] = np.delete(labels[k], energy_indices)
        for k in feature_keys:
            features[k] = np.delete(features[k], energy_indices)
        if "reco" in fin:
            for k in reco_keys:
                reco[k] = np.delete(reco[k], energy_indices)
        weights = np.delete(weights, energy_indices)

#----------

if adjust_tc == True and include_tracks == True and include_cascades == True:
    print("Masking for equal numbers of tracks and cascades")
    no_tracks = len(labels["isTrack"][np.array(labels["isTrack"] == True)])
    no_cascades = len(labels["isCascade"][np.array(labels["isCascade"] == True)])
    ...
    indices = np.argwhere(np.array(labels[cut_label]))
    indices = indices[np.array(range(len(indices))) > min_entries]

    for k in label_keys:
        labels[k] = np.delete(labels[k], indices)
    for k in feature_keys:
        features[k] = np.delete(features[k], indices)
    if "reco" in fin:
        for k in reco_keys:
            reco[k] = np.delete(reco[k], indices)
    weights = np.delete(weights, indices)

#----------

if maxevents != 0:
    print("Only keeping %i events"%maxevents)
    boolarray = np.array(range(len(weights))) < maxevents
    for k in label_keys:
        labels[k] = labels[k][boolarray]
    for k in feature_keys:
        features[k] = features[k][boolarray]
