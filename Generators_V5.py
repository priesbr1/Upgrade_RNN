...
class DataGenerator(keras.utils.Sequence):
    ...
    def __init__(self, hdf5_file, labels=None, maxlen=None, use_log_energy=False, use_weights=False, normal=[], batch_size=256):
        ...
        self.num_batches = int(numpy.floor(self.num_total_entries / batch_size))
        self.batch_info = numpy.zeros(shape=(self.num_batches,2), dtype=numpy.int32)
        for i in range(self.num_batches):
            self.batch_info[i][0] = i*batch_size
            self.batch_info[i][1] = batch_size
        
        # the order in which we will look at the batches
        self.batch_order = numpy.array(range(self.num_batches))

        # shuffle but use the same seed every time (i.e. make this deterministic)
        #rng_state = numpy.random.get_state()
        #numpy.random.seed(0)
        #numpy.random.shuffle(self.batch_order)
        #numpy.random.set_state(rng_state) # re-set RNG state
    ...
    def untransform_labels(self, labels): #label_names=['azimuth', 'entry_energy', 'zenith'], label_scalers=label_scalers):
        ret_labels = numpy.copy(labels)
        for i, k in enumerate(self.label_keys):
            ret_labels[:,i] = ret_labels[:,i]*self.norm[k][1]+self.norm[k][0]
            if k == "energy" and self.use_log_energy: ret_labels[:,i] = 10**ret_labels[:,i]
        return ret_labels
    ...
    def get_reco(self, index):
        ...
        reco = numpy.stack((reco_energy, reco_azimuth, reco_zenith), axis=0)
    ...
    def __getitem__(self, index):
        ...
        if self.use_weights:
            ...
            weights = numpy.ones(batch_size)	    
    
        # load features and stack them into one array
        pmt_index    = self.f['features/pmt_index'][batch_index:batch_index+batch_size]
        ...
        pmt_index    = keras.preprocessing.sequence.pad_sequences(pmt_index,    dtype=pmt_index[0].dtype,    maxlen=self.max_hits)
        ...
        #pulse_charge = numpy.arctan(pulse_charge)/(numpy.pi/2.)
        
        # stack features into one array
        features = numpy.stack((pmt_index, pulse_time, pulse_charge), axis=-1)
        ...
        labels = numpy.zeros(shape=(batch_size, len(self.label_keys)), dtype=numpy.float32)
        for i, k in enumerate(self.label_keys):
            if k == "energy" and self.use_log_energy:
                labels[:,i] = numpy.log10(self.labels[k][batch_index:batch_index+batch_size])
            else:
...
class SplitGenerator(keras.utils.Sequence):
    def __init__(self, input_generator, fraction=1., offset=0.):
        ...
        self.start_offset = int(numpy.floor(self.num_batches * offset))
        self.num_batches_fraction = int(numpy.floor(self.num_batches * fraction))
