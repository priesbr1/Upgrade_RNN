...
from Plots import plot_uncertainty, plot_2dhist, plot_2dhist_contours, plot_1dhist, plot_error, plot_error_contours, plot_loss, plot_error_vs_reco, plot_inputs, plot_outputs
...
def main(config=1):
    ...
    input_pmt_index = Lambda( lambda x: x[:,:,0], name="input_pmt_index" )(input_data) # slice out the pmt index
    ...
    embedding_pmt_index = Embedding(input_dim=vocab_size,
                                    ...
                                    #weights=numpy.concatenate((pmt_positions,pmt_directions),axis=0),trainable=True,
                                    # mask_zero=True,
    ...
    plot_error_contours(np.cos(zenith_true*np.pi/180), np.cos(zenith_predicted*np.pi/180), -1, 1, 'cos(zenith) []', gen_filename=save_folder_name)
